...
@Component
public class ValidateInstitutionEmail implements ValidationInstitution{
    @Autowired
    private InstitutionRepository institutionRepository;
    @Override
    public void validar(DataRegistrationInstitution dataRegistrationInstitution) throws MyException {
        Optional<Institution> optionalInstitution = institutionRepository.findByEmail(dataRegistrationInstitution.email());
        if (optionalInstitution.isPresent()) {
            throw new MyException("Ya existe una escuela registrada con ese email");
        }
    }
}