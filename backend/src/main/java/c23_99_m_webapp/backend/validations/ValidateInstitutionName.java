...
@Component
public class ValidateInstitutionName implements ValidationInstitution{
    @Autowired
    private InstitutionRepository institutionRepository;
    @Override
    public void validar(DataRegistrationInstitution dataRegistrationInstitution) throws MyException {
        Optional<Institution> optionalInstitution = institutionRepository.findByName(dataRegistrationInstitution.name());
        if (optionalInstitution.isPresent()) {
            throw new MyException("Ya existe una escuela registrada con ese nombre");
        }
    }
}
