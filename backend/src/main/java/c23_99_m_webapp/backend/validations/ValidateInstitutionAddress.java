...
@Component
public class ValidateInstitutionAddress implements ValidationInstitution{
    @Autowired
    private InstitutionRepository institutionRepository;
    @Override
    public void validar(DataRegistrationInstitution dataRegistrationInstitution) throws MyException {
        Optional<Institution> optionalInstitution = institutionRepository.findByAddress(dataRegistrationInstitution.address());
        if (optionalInstitution.isPresent()) {
            throw new MyException("Ya existe una escuela registrada en esa dirección");
        }
    }
}
